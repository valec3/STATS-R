...
```{r}
tabla_bi <- table(df_for_analysis$genero, df_for_analysis$grado_severidad)
chisq.test(tabla_bi)

```
Resultado:
χ²(2, n = 57,212) = 42.039, p = 7.437e-10.

Decisión: Como p < 0.05, se rechaza H0.

Interpretación: Existe evidencia estadísticamente significativa de que el género y el grado de severidad de la anemia no son independientes. Esto significa que la severidad (leve, moderada, severa) se distribuye de forma diferente entre niños y niñas.

Nota: El test chi-cuadrado indica asociación, pero no muestra la magnitud ni dirección de esa asociación.
...
```{r}
df_for_analysis %>% 
  anova_test(edad_meses ~ grado_severidad)

```

Resultados del ANOVA: F(2, 57,209) = 319.31, p < 0.001, ges = 0.011

Decisión: Se rechaza H0.

Interpretación: Existen diferencias estadísticamente significativas en la edad promedio entre los grupos de severidad. Sin embargo, el tamaño del efecto es muy pequeño (ges = 0.011). Esto indica que la edad tiene una influencia muy débil sobre la severidad de la anemia, sin relevancia clínica considerable.
...
cor.test(df_for_analysis$edad_meses, df_for_analysis$cantidad, method = "pearson")

```

Resultados del test de correlación de Pearson: r = -0.0038, IC95% [-0.0120, 0.0044], t = -0.91, gl = 57,210, p = 0.360

Decisión: No se rechaza H0.

Interpretación: No existe correlación estadísticamente significativa entre la edad y la cantidad de diagnósticos. El coeficiente de correlación es cercano a 0, lo que indica ausencia de relación lineal entre ambas variables.
